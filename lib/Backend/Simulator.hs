...
run machine = case fetchInstr machine of
  ...
  Just instr ->
    ...
    in if not halt then machine' |> incrIP |> run else machine'

getMemoryDump :: Machine -> [(Int, String)]
getMemoryDump machine = HM.toList (memory machine)
  ...
  in
    if ipVal < 2056
      then
        ...
        in
          if instrIdx < (length $ library machine)
            then Just $ (library machine) !! instrIdx
            else Nothing
      else
        let instrIdx = (ipVal - 2056) `div` 2
        in
          if instrIdx < (length $ code machine)
...
setRegVal reg !val machine = machine
  { registers = HM.insert (fromEnum reg) (toXSMStr val) (registers machine)
  }
...
setMemory !loc !val machine = if loc < 1024
  then error
    ("Set memory < 1024: " ++ (show loc) ++ " <- " ++ (toXSMStr val))
...
execute !instr !machine = case instr of
  ...
  _          -> (, False) $ case instr of
    XSM_NOP -> machine
    XSM_BRKP -> machine -- seq (unsafePerformIO $ printDumpState machine) machine
    XSM_MOV_R r1 r2 ->
    ...
    XSM_CALL loc ->
      ...
      in
        machine { ip = loc - 2 } |> setRegVal SP (spVal + 1) |> setMemory
          (spVal + 1)
          (ip machine + 2)
    XSM_CALLInd reg ->
      let
    ...
    XSM_RET ->
      ...
      in machine { ip = ipSav - 2 } |> setRegVal SP (spVal - 1)
...
cmpSet r1 opI opS r2 machine =
  ...
  let
    r1Val = getRegVal r1 machine
    r2Val = getRegVal r2 machine
  in case (readMaybe r1Val, readMaybe r2Val) :: (Maybe Int, Maybe Int) of
...
printDumpState :: Machine -> IO ()
printDumpState machine = do
  putStrLn "Registers: "
  printRegisterDump machine
  putStrLn "Memory: "
  printMemoryDump machine

printRegisterDump :: Machine -> IO ()
printRegisterDump machine = do
  let regdump = sortOn fst (HM.toList $ registers machine)
  mapM_ (\(reg, val) -> putStrLn $ (show $ toEnum @Reg reg) ++ ": " ++ val) regdump

printMemoryDump :: Machine -> IO ()
printMemoryDump machine = do
  let memdump = sortOn fst (getMemoryDump machine)
