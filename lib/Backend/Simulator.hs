...
getRegVal reg =
  fromMaybe "" . HM.lookup (fromEnum reg) . registers

setRegVal :: (XSMStr a) => Reg -> a -> Machine -> Machine
setRegVal reg val machine = machine
  { registers = HM.insert (fromEnum reg)
                          (toXSMStr val)
                          (registers machine)
  }
...
setMemory loc val machine = machine
  { memory = HM.insert loc (toXSMStr val) (memory machine)
  }
...
cmpSet r1 opI opS r2 machine =
  ...
  in
    case
      (readMaybe r1Val, readMaybe r2Val) :: ( Maybe Int
      , Maybe Int
      )
    of
      (Just r1Val', Just r2Val') ->
        let cmp =
                if r1Val' `opI` r2Val' then 1 else 0 :: Int
...
syscall num machine =
  let
    ...
    args =
      [ getMemory (spVal - i) machine | i <- [4, 3 .. 0] ]
  in
    case num of
      ...
      _ ->
        error $ "Syscall not implemented: " ++ (show num)
