...
mkStmtAssign lhs rhs span = do
  ...
  unless (length dims == 0) $ throwError $ errArrayNotAllowed span lhsDeclSpan
  unless (lhsType == rhsType) $ throwError $
    errTypeMismatch
      lhsType
      lhsDeclSpan
      rhsType
...
mkStmtBreak ::
  (MonadError Error m, ReadLoopStack m) =>
  Span ->
  m StmtBreak
mkStmtBreak span = do
  ...
  where
    throwOutOfLoop =
      throwError (Error.syntaxError span)
  ...
  where
    throwFuncRedeclared func = case func of
      ...
      Just (FuncDeclared funcDec) ->
        throwError $
          errFuncRedeclared span (funcDeclSpan funcDec)
      Just (FuncDefined funcDec _) ->
        throwError $
          errFuncRedeclared span (funcDeclSpan (funcDec))

doFuncDefine ::
  (MonadError Error m, WriteFuncs m, WriteSymbols m, SymbolTableStack m) =>
  PrimitiveType ->
  String ->
  ...
  where
    throwRedefined maybeFunc = case maybeFunc of
      ...
      Just (FuncDeclared func') -> do
        return $ Just func'
    throwMismatch retType name args maybeFuncDec =
      case maybeFuncDec of
        Nothing -> return Nothing
        Just (funcDecl) -> do
          unless
            ( and
...
mkLValue (SpanW identName identSpan) indices = do
  sym <- symLookup identName >>= \case
    Nothing ->
      throwError
        (errIdentifierNotDeclared identName identSpan)
    Just sym -> return sym
  let DataType dims _ = symDataType sym
  unless (length indices <= length dims) $
    throwError
      (Error.customError "Too much indices" identSpan) -- TODO Better error
  indices' <-
    mapM
      ( \(SpanW index indexSpan) -> do
          indexType <- rValueDataType index
          unless (indexType == DataType [] TypeInt) $
            throwError
              ( Error.customError
                  "mkArrayIndex: index not int"
                  indexSpan
              ) -- TODO Better error
          return $ index
      )
...
mkExpFuncCall funcName args span = do
  ...
  where
    throwFuncNotDeclared sym = case sym of
      Nothing ->
        throwError $ errFuncNotDeclared funcName span
      Just sym -> return sym
...
errTypeMismatch ::
  DataType -> Span -> DataType -> Span -> Error
...
errFuncDefMismatch span declSpan =
  [ ( "Function definition and declarations are different",
      span
    ),
    ("Was declared here", declSpan)
  ]
